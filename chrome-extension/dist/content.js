...
`);n.textContent=s,n.scrollTop=n.scrollHeight}async handleSendMessage(t,e){var s,a,r,i;const o=t.value.trim();if(!o)return;this.addMessage(e,o,"user"),t.value="";const n=this.addLoadingMessage(e);try{const l=this.getVideoId(),g=this.getCurrentTimestamp();console.log("🎓 VideoTutor: Sending query with videoId:",l,"message:",o);const y=await fetch("http://localhost:8000/query",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({videoId:l,timestamp:g,prompt:o})});if(n.remove(),!y.ok)throw new Error(`HTTP ${y.status}: ${y.statusText}`);const d=(s=y.body)==null?void 0:s.getReader();let c="";const p=this.addMessage(e,"","assistant");for(;d;){const{done:u,value:v}=await d.read();if(u)break;const T=new TextDecoder().decode(v).split(`
`);for(const w of T)if(w.startsWith("data: ")){const b=w.slice(6);if(b==="[DONE]"){const h=p.querySelector("div:last-child");if(h&&c){const f=this.parseMarkdown(c);console.log("🎓 FINAL Setting innerHTML to:",f),h.innerHTML=f}break}try{const h=JSON.parse(b);if((i=(r=(a=h.choices)==null?void 0:a[0])==null?void 0:r.delta)!=null&&i.content){c+=h.choices[0].delta.content;const f=p.querySelector("div:last-child");if(f){const k=this.parseMarkdown(c);console.log("🎓 Setting innerHTML to:",k),f.innerHTML=k}}}catch{}}}const m=p.querySelector("div:last-child");if(m&&c){const u=this.parseMarkdown(c);console.log("🎓 BACKUP Final Setting innerHTML to:",u),m.innerHTML=u}}catch(l){console.error("🎓 VideoTutor: Error sending message:",l),n.parentNode&&n.remove();const g=l instanceof Error?l.message:"Unknown error";this.addMessage(e,`Error: ${g}`,"error")}}addLoadingMessage(t){const e=document.createElement("div");e.className="message-loading slide-in-left",e.style.cssText=`
      padding: 16px 20px;
      margin-bottom: 12px;
      font-size: 14px;
      line-height: 1.5;
    ...
    `;for(let r=0;r<3;r++){const i=document.createElement("div");i.style.cssText=`
      ...
      `,a.appendChild(i)}return n.appendChild(s),n.appendChild(a),e.appendChild(o),e.appendChild(n),t.appendChild(e),t.scrollTop=t.scrollHeight,e}parseMarkdown(t){console.log("🎓 BEFORE parsing:",t);let e=t;(e.startsWith("```markdown")||e.startsWith("```"))&&(e=e.replace(/^```(markdown)?\s*/,"").replace(/```\s*$/,""));let o=e.replace(/^### (.+)$/gm,'<h3 style="margin: 4px 0 2px 0; font-size: 16px; font-weight: 600; color: var(--text-primary);">$1</h3>').replace(/^## (.+)$/gm,'<h2 style="margin: 5px 0 3px 0; font-size: 18px; font-weight: 600; color: var(--text-primary);">$1</h2>').replace(/^# (.+)$/gm,'<h1 style="margin: 6px 0 4px 0; font-size: 20px; font-weight: 600; color: var(--text-primary);">$1</h1>').replace(/\*\*(.+?)\*\*/g,'<strong style="font-weight: 600; color: var(--text-primary);">$1</strong>').replace(/`(.+?)`/g,'<code style="background: rgba(255,255,255,0.1); padding: 2px 6px; border-radius: 4px; font-family: monospace; font-size: 13px; color: var(--text-primary);">$1</code>').replace(/^- (.+)$/gm,'<div style="margin: 2px 0; padding-left: 12px; position: relative;"><span style="position: absolute; left: 0; color: var(--text-primary);">•</span>$1</div>').replace(/\n/g,"<br>");return console.log("🎓 AFTER parsing:",o),o}addMessage(t,e,o){const n=document.createElement("div"),s=o==="user"?"slide-in-right":"slide-in-left";if(n.className=`message-${o} ${s}`,o==="user")n.style.cssText=`
        padding: 12px 18px;
        margin-bottom: 12px;
        font-size: 14px;
        line-height: 1.5;
      ...
      `;const r=document.createElement("div");r.style.cssText=`
        flex: 1;
        padding-top: 2px;
      `,r.innerHTML=this.parseMarkdown(e),n.appendChild(a),n.appendChild(r)}else n.style.cssText=`
        padding: 12px 18px;
        margin-bottom: 12px;
        font-size: 14px;
        line-height: 1.5;
